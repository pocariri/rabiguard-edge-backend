    ...
    def yolo_worker(self):
        ...
        while not stop_event.is_set():
            ...
            try:
                ...
                # 최적화: 해상도를 320에서 192로 더 축소하여 CPU 부하를 극한으로 낮춤
                frame_small = cv2.resize(frame_bgr, (192, 192))
                
                depth_map = None
                if depth_raw is not None:
                    depth_map = np.array(depth_raw, dtype=np.float32).reshape((256, 320))
                
                # NCNN 추론 (축소된 192x192 이미지 사용)
                ...
                if results[0].boxes is not None and results[0].boxes.id is not None:
                    ...
                    for box, track_id in zip(boxes_small, track_ids):
                        ...
                        # 192x192 좌표를 원래 해상도로 복원
                        x1 = int(box[0] * (w_orig / 192))
                        y1 = int(box[1] * (h_orig / 192))
                        x2 = int(box[2] * (w_orig / 192))
                        y2 = int(box[3] * (h_orig / 192))
